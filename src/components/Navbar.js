...
export default function Navbar(props) {
    const menus = [
        ...
        {name: "Wiki", url: "https://wiki.forgottenworld.it/mc/Forgotten_World", action: null, pcOnly: false},
        ...
        {name: "Sostienici", url: null, action: 2, pcOnly: false} //2
    ]
  
    return (
        <div className="navbar">
            {
                menus.map((m, i) => <NavbarLink pageSetter={props.pageSetter} 
                                                key={"nbl"+i}
                                                name={m.name} 
                                                pcOnly={m.pcOnly} 
                                                url={m.url} 
